...
void thread_leitura_sensores(Buffer_Circular* buffer, std::atomic<bool>& running, float &pos_x, float &pos_y, bool &modo_auto, bool &defeito);

// Thread para armazenar/logar os dados lidos 
void thread_armazena(std::atomic<bool>& running, int id_caminhao, float &pos_x, float &pos_y, bool &modo_auto, bool &defeito, int &leitor_interface_local);
